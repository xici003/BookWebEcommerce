    ...
    public class AppDBInitializer
    {
		...
		public static async Task SeedUsersAndRolesAsync(IApplicationBuilder applicationBuilder)
		{
			using (var serviceScope = applicationBuilder.ApplicationServices.CreateScope())
			{
				...
				if (!await roleManager.RoleExistsAsync(UserRoles.Admin)) // Kiểm tra xem vai trò Admin và User đã tồn tại hay chưa
					await roleManager.CreateAsync(new IdentityRole(UserRoles.Admin));// Nếu chưa thì thêm vào
